#!/usr/bin/env bash
# Domoticz XAir Plugins
# (c) 2018 by xiaoyao9184
# https://github.com/xiaoyao9184/Domoticz-XAir-Plugin
# Installs Domoticz XAir Plugins
#
# Domoticz XAir Plugins is free software: you can redistribute it and/or modify
...
Dest_folder=""
Temp_folder="/tmp/Domoticz-XAir-Plugin"
...
if [[ ${EUID} -eq 0 ]]; then
    ...
    echo "::: Script called with non-root privileges. The Domoticz XAir Plugin installs server packages and configures"
    ...
    if [ -x "$(command -v sudo)" ]; then
        ...
        exec curl -sSL https://github.com/xiaoyao9184/Domoticz-XAir-Plugin/raw/master/install.sh | sudo bash "$@"
        exit $?
    else
...
welcomeDialogs() {
    ...
    whiptail --msgbox --backtitle "Welcome" --title "Domoticz XAir Plugin automated installer" "\n\nThis installer will install xiaomi plugin to your Domoticz!\n\n
Domoticz XAir Plugin is free\n\n
Domoticz XAir Plugin connect to the detector using USB" ${r} ${c}
}
...
Github:  https://github.com/xiaoyao9184/Domoticz-XAir-Plugin" ${r} ${c}
}

chooseServices() {
    cd "$Temp_folder/Domoticz-XAir-Plugin"
    index=0
    indexs=()
    options=()
    for d in */; do
    ...
    echo "::: Find ${#indexs[*]} plugins in $Temp_folder/Domoticz-XAir-Plugin"
...
downloadDomoticzPlugin() {
    ...
    # Get plugin
    echo "::: Clone github"
    git clone https://github.com/xiaoyao9184/Domoticz-XAir-Plugin &> /dev/null
    cd "./Domoticz-XAir-Plugin"
}
